      ...
      <ButtonGroup size="sm" style={{ marginLeft: 0, marginTop: -16 }}>
        ...
        <Dropdown style={{ borderRadius: 0 }}>
          ...
          <Dropdown.Menu>
            <Dropdown.Item
              eventKey="1"
              onClick={() => handleShow(item.id)}  // Pass the item.id here
              style={{ borderRadius: 0, fontSize: '12px' }}
            >
              {isVisible ? <IoMdRemoveCircleOutline size={18} color='red' /> : <IoIosAddCircleOutline size={18} color='green' />} Utilities
            </Dropdown.Item>
